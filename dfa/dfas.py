...
class DFA_ARGS(TypedDict):
    transition_table: dict[str: list[int]]
    name: str
...
class DFA:
    ...
    def __init__(self, **kwargs: DFA_ARGS) -> None:
    ...
    def process_token(self, token: str) -> bool:
        ...
        for i in range(len(token)):
            if token[i] in self.alphabets["#"] and token[i] not in self.transition_table:
                newToken += "#"
